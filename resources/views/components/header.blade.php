<header x-data="{ isOpen: false }">
        ...
        <div class="flex justify-between items-center gap-4 py-4 md:py-6 box-border">
            <div class="flex items-end gap-4">
                <button x-on:click="$dispatch('open-modal', 'mobile-nav')"
                    ...
                    <x-svg-bars3 class="w-6 h-6 text-neutral-500 group-hover:text-black transition-all duration-200" />
                </button>
                <a href="{{ route('home') }}">
    ...
    <template x-teleport="#aside-modal" x-show="isOpen">
        <x-modal name="mobile-nav" :maxWidth="null" :fullscreen="true">
            <x-modal-card class="w-screen min-h-screen rounded-none pt-0 px-0 sm:p-0 flex flex-col"
                          x-effect="window.addEventListener('resize', () => { if (window.innerWidth > 768) $dispatch('close-modal', 'mobile-nav') })">
                <div class="flex items-end gap-4 border-b py-5 px-4">
                    <button x-on:click="$dispatch('close-modal', 'mobile-nav')"
                        ...
                        <x-svg-x-mark class="w-6 h-6 text-neutral-500 group-hover:text-black transition-all duration-200" />
                    </button>
                    <a href="{{ route('home') }}">
                        <span class="sr-only">{{ config('app.name') }}</span>
                        <img src="{{ asset('images/sq1-logo.svg') }}" alt="{{ config('app.name') }}" width="120" height="37" />
